...
bot = Bot("gqlrnqmjticuhgnvurlzgrkbnuttudrk")
target = "g0DCcPk0f01d2db5294506d525654b53"
...
def alert(guid,user,alert_text=""):
	...
	max_alert = 1    # you can change it


	if alert_count == max_alert:
		blacklist.append(guid)
		bot.sendMessage(target, "\n 🚫 کاربر [ @"+user+" ] \n شما به علت تبلیغ ، از گروه اخراج میشود .", msg["message_id"])
				...
				if not sleeped:
					...
					if msg["type"]=="Text" and not msg["message_id"] in answered:
						...
						if msg["author_object_guid"] in admins:
							...
							if msg["text"] == "سلام" or msg["text"] == "صلام" or msg["text"] == "صلم" or msg["text"] == "سلم" :
								bot.sendMiessage(target,"𝙃𝙞 𝘼𝙙𝙢𝙞𝙣:)", msg["message_id"])
							
							if msg["text"] == "ربات خاموش" or msg["text"] == "/sleep" :
								sleeped = True
								bot.sendMiessage(target, "💤 ربات اکنون خاموش است .", msg["message_id"])


							elif msg["text"] == "!start" or msg["text"] == "/start" :
								bot.sendMiessage(target, "✨ به گیگابات خوش آمدید .\n\nبرای مشاهده لیست دستورات ربات ، کلمه (دستورات) را ارسال کنید .", msg["message_id"])
								...
								try:
									...
									bot.sendMiessage(target, "✅ ذخیره شد", msg["message_id"])
								except:
									bot.sendMiessage(target, "❌ خطا در اجرای دستور", msg["message_id"])
								...
								try:
									...
									if not guid in admins :
										...
										bot.sendMiessage(target, "✅ کاربر @"+ str(user) +" با موفقیت ادمین شد .", msg["message_id"])
									else:
										bot.sendMiessage(target, "❌ کاربر هم اکنون ادمین میباشد", msg["message_id"])
								...
								try:
									...
									if guid in admins :
										...
										bot.sendMiessage(target, "✅ کاربر @"+ str(user) +" با موفقیت از ادمینی برکنار شد .", msg["message_id"])
									else:
										bot.sendMiessage(target, "❌ کاربر ادمین گروه نمیباشد", msg["message_id"])
								...
								try:
									...
									if not guid in admins :
										if not guid in exemption:
											...
											bot.sendMiessage(target, "✅ کاربر با موفقیت معاف شد .", msg["message_id"])
										else:
											bot.sendMiessage(target, "❌ کاربر هم اکنون معاف میباشد .", msg["message_id"])
								
									else :
										bot.sendMiessage(target, "❌ کاربر ادمین میباشد .", msg["message_id"])
								...
								try:
									...
									if not guid in admins :
										if guid in exemption:
											...
											bot.sendMiessage(target, "✅ کاربر از معافیت حذف شد", msg["message_id"])
										else:
											bot.sendMiessage(target, "❌ کاربر معاف نمیباشد .", msg["message_id"])
									else :
										bot.sendMiessage(target, "❌ کاربر ادمین میباشد", msg["message_id"])
									...
									try:
										...
										if not guid in admins and guid in exemption:
											if guid in exemption:
												exemption.remove(guid)
												bot.sendMiessage(target, "✅ کاربر با از معافیت حذف شد", msg["message_id"])
											else:
												bot.sendMiessage(target, "❌ کاربر معاف نمیباشد .", msg["message_id"])
								...
								try:
									...
									bot.sendMiessage(target, text + str(stars_list), msg["message_id"])
								except:
									bot.sendMiessage(target, "❌ خطا در اجرای دستور", msg["message_id"])
								...
								try:
									...
									bot.sendMiessage(target, text + str(alert_list), msg["message_id"])
								except:
									bot.sendMiessage(target, "❌ خطا در اجرای دستور", msg["message_id"])
								...
								try:
									...
									if guid in no_alerts:
										...
										alerts[user] = 0
										bot.sendMiessage(target, "✅ اخطارهای کاربر حذف شد .", msg["message_id"])
									else:
										bot.sendMiessage(target, "❌ کاربر اخطاری ندارد .", msg["message_id"])
									...
									try:
										...
										bot.sendMiessage(target, "❌ لطفا دستور را به درستی وارد کنید", msg["message_id"])
								...
								try:
									...
									bot.sendMiessage(target, "⏰ حالت آرام برای "+str(number)+"ثانیه فعال شد", msg["message_id"])
								...
								try:
									...
									bot.sendMiessage(target, "⏰ حالت آرام غیرفعال شد", msg["message_id"])
								except:
									bot.sendMiessage(target, "❌ خطا در اجرای دستور", msg["message_id"])
								
							# elif msg["text"] == "قفل گیف" or msg["text"] == "/gif_lock":
							# 	gif_lock = True
							# 	bot.sendMiessage(target, "✅ قفل گیف و استیکر فعال شد .", msg["message_id"])

							
							# elif msg["text"] == "حذف قفل گیف" or msg["text"] == "/del_gif_lock":
							# 	gif_lock = False
							# 	bot.sendMiessage(target, "✅ قفل گیف و استیکر غیرفعال شد .", msg["message_id"])
								...
								auto_lock = False
								bot.sendMiessage(target, "🔓 قفل خودکار برداشته شد .", msg["message_id"])
								...
								try:
									...
									if not guid in admins :
										bot.banGroupMember(target, guid)
										# bot.sendMiessage(target, "✅ کاربر با موفقیت از گروه اخراج شد", msg["message_id"])
									else :
										bot.sendMiessage(target, "❌ کاربر ادمین میباشد", msg["message_id"])
								...
								try:
									...
									if number > 50:
										bot.sendMiessage(target, "❌ ربات فقط تا 50 پیام اخیر را پاک میکند .", msg["message_id"])
									else:
										answered.reverse()
										bot.deleteMessages(target, answered[0:number])

										bot.sendMiessage(target, "✅ "+ str(number) +" پیام اخیر با موفقیت حذف شد", msg["message_id"])
										answered.reverse()

								except:
									try:
										bot.deleteMessages(target, [msg["reply_to_message_id"]])
										bot.sendMiessage(target, "✅ پیام با موفقیت حذف شد", msg["message_id"])
									except:
										bot.sendMiessage(target, "❌ خطا در اجرای دستور", msg["message_id"])
								...
								bot.sendMiessage(target, "✅  قوانین بروزرسانی شد", msg["message_id"])
								# rules.close()
								...
								bot.setMembersAccess(target, ["AddMember"])
								bot.sendMiessage(target, "🔒 گروه قفل شد", msg["message_id"])


							elif msg["text"] == "بازکردن گروه" or msg["text"] == "/unlock" :
								bot.setMembersAccess(target, ["SendMessages","AddMember"])
								bot.sendMiessage(target, "🔓 گروه اکنون باز است", msg["message_id"])
								...
								bot.sendMiessage(target, str(rules), msg["message_id"])
								# rules.close()
							
							elif msg["text"] == "دستورات":
								commands = open("commands.txt","r",encoding='utf-8').read()
								bot.sendMiessage(target,str(commands),msg["message_id"])
						...
						# User Commands
							...
							if hasAds(msg["text"]) and not msg["author_object_guid"] in exemption:
							...
							elif msg["text"].startswith("ازاد"):
								guid = bot.getInfoByUsername(msg["text"].replace("معاف ","")[1:])["data"]["chat"]["abs_object"]["object_guid"]
								if not guid in admins :
									if not guid in exemption:
										bot.unbanMember(target, guid)
										bot.sendMiessage(target,"اوک",msg["message_id"])
							
							elif msg["text"].startswith("فال"):
								response = post("http://api.codebazan.ir/fal/index.php/pic").content
								with open("shot.jpg","wb") as shot: shot.write(response)
								bot.sendPhoto(target, "./shot.jpg", [720,40], caption="فال حافظ اماده شد آذرخش")
							
							elif msg["text"].startswith("سلام") or msg["text"].startswith("صلم") or msg["text"].startswith("صلام") or msg["text"].startswith("سلم") or msg["text"].startswith("ثلام") or msg["text"].startswith("ثلم"):
								bot.sendMiessage(target,get,msg["message_id"])
							
							elif msg["text"].startswith("فاک") or msg["text"].startswith("صلم") or msg["text"].startswith("صلام") or msg["text"].startswith("سلم") or msg["text"].startswith("ثلام") or msg["text"].startswith("ثلم"):
								#bot.sendMiessage(target,get,msg["message_id"])
								bot.deleteMessages(target, [str(msg("message_id"))])
								...
								try:
									guid = bot.getInfoByUsername(msg["text"].replace("افزودن ","").replace("/add ","")[1:])["data"]["chat"]["object_guid"]
									if guid in blacklist:
										bot.sendMiessage(target, "❌ کاربر در لیست سیاه میباشد و فقط ادمین میتواند فرد را به گروه اضافه کند .", msg["message_id"])
									else:
										bot.invite(target, [guid])
										# bot.sendMiessage(target, "✅ کاربر اکنون عضو گروه است", msg["message_id"])
								...
								bot.sendMiessage(target, "🔗 لینک گروه :\n"+str(group), msg["message_id"])
                                    
							for i in data.keys():
								if i == msg["text"]:
									bot.sendMiessage(target, str(data[i]), msg["message_id"])
						...
						if data["type"]=="RemoveGroupMembers":
							...
							#bot.sendMiessage(target, f"🚨 کاربر {user} با موفقیت از گروه حذف شد .", msg["message_id"])
							bot.deleteMessages(target, [msg["message_id"]])
						
						elif data["type"]=="AddedGroupMembers":
							user = bot.getUserInfo(data['peer_objects'][0]['object_guid'])["data"]["user"]["first_name"]
							#bot.sendMiessage(target, f"سلام {user} عزیز 🌹 \n • به گروه {name} خوش اومدی 😍 \n 📿 لطفا قوانین رو رعایت کن .\n 💎 برای مشاهده قوانین کافیه کلمه (قوانین) رو ارسال کنی .", msg["message_id"])
							bot.deleteMessages(target, [msg["message_id"]])
						
						elif data["type"]=="LeaveGroup":
							user = bot.getUserInfo(data['performer_object']['object_guid'])["data"]["user"]["first_name"]
							#bot.sendMiessage(target, f"خدانگهدار {user} 👋 ", msg["message_id"])
							bot.deleteMessages(target, [msg["message_id"]])
					...
					if msg["text"] == "ربات روشن" or msg["text"] == "/wakeup":
						sleeped = False
						bot.sendMiessage(target, "✅ ربات اکنون روشن است .", msg["message_id"])
